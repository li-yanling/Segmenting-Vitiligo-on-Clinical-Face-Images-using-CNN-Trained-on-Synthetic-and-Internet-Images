...
for ii=1:380
    ...
    [path,name,ext] = fileparts(imgList_test_ori(ii).name);
    test_image=I_ori;
    %shape
    origin_img=I;
    I=uint8(I);
    ...
    height=round(0.1*vector(1,4));
    width=round(0.1*vector(1,3));
...
while(isempty(col_final))
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%repeat for the second, symmetry region;
    ...
    %generate mask
    save_path_ori = 'C:\Users\yanling\Documents\vitiligo dataset\20200924_revision\chicago_for_GAN\chicago_512\';
    basename1=[name,'.png'];
    ori_name =fullfile(save_path_ori,basename1);
    save_path_skin = "C:\Users\yanling\Documents\vitiligo dataset\20200924_revision\small_patch_01\only_skin\";
    skin_name = fullfile(save_path_skin,basename1);
    save_path_gen = 'C:\Users\yanling\Documents\vitiligo dataset\20200924_revision\small_patch_01\gen\';
    name_size = '_01';
    basename2=[name,name_size,'.png'];
    simu_name =fullfile(save_path_gen,basename2);
    save_path_mask = 'C:\Users\yanling\Documents\vitiligo dataset\20200924_revision\small_patch_01\mask\';
    mask_name = fullfile(save_path_mask,basename2);
    mask=mask./255;
    mask=imresize(mask,[512,512]);
    imwrite(mask, mask_name)
   % imwrite(I_ori,ori_name)
    J_skin_resize = imresize(J_skin,[512,512]);
    imwrite(J_skin_resize,skin_name);
    imwrite(test_image,simu_name)
%     figure
%     subplot(1,2,1)
%     imshow(test_image)
%     subplot(1,2,2)
